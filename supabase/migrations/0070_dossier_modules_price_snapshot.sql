-- ============================================================================
-- 0070 — Snapshot du prix par module dans le dossier
-- ============================================================================
-- dossier_modules fige déjà title_snapshot + duration_hours au moment de la
-- création (le catalogue peut évoluer ensuite). Le prix ne l'était pas : un
-- changement de modules.price_cents modifiait rétroactivement le coût d'un
-- dossier déjà monté. On ajoute price_cents (gelé) et on adapte save_dossier :
--   - création  : prix explicite du client, sinon prix catalogue courant (gel)
--   - ré-enregistrement : on conserve le snapshot existant sauf prix explicite

ALTER TABLE app.dossier_modules
  ADD COLUMN IF NOT EXISTS price_cents BIGINT CHECK (price_cents IS NULL OR price_cents >= 0);

-- Backfill : photographie le prix catalogue courant pour les lignes existantes.
UPDATE app.dossier_modules dm
SET price_cents = m.price_cents
FROM app.modules m
WHERE m.id = dm.module_id
  AND dm.price_cents IS NULL
  AND m.price_cents IS NOT NULL;

-- ---------------------------------------------------------------------------
-- save_dossier : ajout du gel de prix dans la boucle modules
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF p_dossier ? 'modules' THEN
    ...
    LOOP
      -- Prix explicitement fourni par le client (NULL si absent).
      v_price_raw := NULLIF(v_module->>'price_cents','')::bigint;

      INSERT INTO app.dossier_modules (
        id, organization_id, dossier_id, module_id, position, title_snapshot,
        duration_hours, start_date, end_date, price_cents
      )
      VALUES (
        ...
        -- Gel : prix client, sinon prix catalogue courant.
        COALESCE(
          v_price_raw,
          (SELECT m.price_cents FROM app.modules m WHERE m.id = (v_module->>'module_id')::uuid)
        )
      )
      ON CONFLICT (id) DO UPDATE SET
        ...
        start_date     = EXCLUDED.start_date,
        end_date       = EXCLUDED.end_date,
        -- On ne ré-écrase le snapshot que si un prix explicite est fourni.
        price_cents    = COALESCE(v_price_raw, app.dossier_modules.price_cents),
        updated_at     = now();
    END LOOP;
  END IF;
