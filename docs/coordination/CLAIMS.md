...
|----------|------------------|---------|--------|--------|
| Opus (e-signature) | E-signature de documents par lien token : helper `shared/lib/document-signature-token.ts`, demande staff `documents/[id]/apercu/signature-actions.ts` + panneau, page publique `(apprenant)/signer/document/[token]` + canvas + action soumission, statut/audit via `document_signatures` existant. **Sans migration.** | feat/document-esignature | 2026-06-18 | en cours |
