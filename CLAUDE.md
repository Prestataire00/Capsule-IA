...
Chaque écran est `command` ou `workflow` — déclaré en haut du fichier. Palette : zinc neutre + **orange-500 brand** + accents fonctionnels (rose pour humains, blue pour sessions, purple pour Qualiopi, emerald/amber/red pour statuts). Tailles : 11/12/13/15/17/20/24px (+ 30px hero). **Poids** : font-normal/medium par défaut, font-semibold autorisé sur titres hero h1/h2, font-bold interdit. Max 1 bouton primaire orange par écran. shadow-sm partout, shadow-md hover, shadow-lg sur hero unique, xl+ interdit. Gradients chaleureux autorisés sur hero. **Illustrations SVG encouragées** (empty states, hero). **Emoji autorisé en décoration ponctuelle** quand porteur de sens (1-2/écran max). Border-radius rounded-2xl autorisé sur hero cards. Mode sombre obligatoire. **v4 « Horizon couleur » (prime sur le reste)** : police Manrope, `zinc` remappé en gris bleu nuit et `violet` remappé en orange dans `tailwind.config.ts`, barre latérale orange d'origine + onglets de section, noms de session en orange (`var(--sess)`), une couleur d'identité par formation (`shared/lib/formation-color.ts`, jamais orange), tableaux une ligne par objet avec colonnes métier, pastilles pleines, graphiques, aucun emoji. **v4.1 « vivant »** : de la couleur partout où elle porte un sens — `KpiCard`/`StatCard` colorés (`shared/ui/kpi-card.tsx`), icônes dans des carrés `ACCENTS[x].soft`, `AccentBar`, avatars colorés ; code couleur personnes rose, sessions/dates bleu, argent/validé vert, Qualiopi violet, alertes ambre, actions orange. Mono (Geist Mono) réservé aux identifiants — **dates, heures, montants et compteurs en `tabular-nums`, jamais en `font-mono`**. Détails dans `.cursor/rules/70-ui-charter.mdc`.
