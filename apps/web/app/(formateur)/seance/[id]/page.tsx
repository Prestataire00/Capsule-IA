...
import { ClipboardList, ListChecks, PenLine, Mail, Phone, Users, Building2, Home, BookOpen, MessagesSquare } from 'lucide-react';
...
function Raccourci({
  ...
  return (
    <Link
      href={href}
      className="rounded-xl border border-zinc-200/70 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-3 hover:border-orange-300 dark:hover:border-orange-900/60 transition flex items-center gap-3"
    >
      <span className="w-8 h-8 rounded-md grid place-items-center bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 flex-shrink-0">
        <Icone className="w-4 h-4" />
      </span>
      <span className="min-w-0">
        <span className="block text-[13px] font-medium text-zinc-900 dark:text-zinc-100 truncate">{label}</span>
  ...
  return (
    <div className="max-w-2xl w-full mx-auto px-4 py-6 space-y-6">
      ...
      <section className="space-y-2">
        <h2 className="text-[13px] font-medium text-zinc-500 dark:text-zinc-400 uppercase tracking-wider">Ma séance</h2>
        <div className="grid sm:grid-cols-2 gap-2">
          <Raccourci
            href={`/seance/${params.id}/supports`}
            icone={BookOpen}
            label="Supports de cours"
            detail={supports.length === 0 ? 'Aucun support déposé' : `${supports.length} support${supports.length > 1 ? 's' : ''} · ${publies} publié${publies > 1 ? 's' : ''}`}
          />
          <Raccourci
          ...
          />
          <Raccourci href={`/seance/${params.id}/fiches-besoin`} icone={ClipboardList} label="Fiches besoin" />
          <Raccourci href={`/seance/${params.id}/questionnaires`} icone={ListChecks} label="Questionnaires" />
          <Raccourci href={`/emarger/${params.id}`} icone={PenLine} label="Émargement" detail="Faire signer et clôturer" />
        </div>
      </section>
    </div>
  );
