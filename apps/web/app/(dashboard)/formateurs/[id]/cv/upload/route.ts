...
// Bucket privé (un CV est une donnée personnelle) : la lecture se fait par URL
// signée depuis la fiche formateur, jamais par une URL publique.
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const path = `${trainer.organization_id}/${trainerId}/cv.${ext}`;

  const { error: upErr } = await supabase.storage
    .from(BUCKET)
    .upload(path, await raw.arrayBuffer(), { contentType: raw.type, upsert: true });
  if (upErr) {
    console.error('[cv/upload]', upErr);
  ...
  const { error: updErr } = await supabase
    .schema('app')
    .from('trainers')
    .update({ cv_path: path } as never)
    .eq('id', trainerId);
  if (updErr) {
    console.error('[cv/upload] update', updErr);
