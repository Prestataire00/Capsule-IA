// ARCHETYPE: command
import Link from 'next/link';
import { Plus } from 'lucide-react';
import { funders } from '@/shared/mock/data';
import { SectionLabel } from '@/shared/ui/section-label';

const labels: Record<string, string> = {
  opco: 'OPCO',
  cpf: 'CPF',
  pole_emploi: 'France Travail',
  region: 'Région',
  autofinancement: 'Autofinancement',
  entreprise: 'Entreprise',
  autre: 'Autre',
};

export default function FinanceursPage() {
  return (
    <div className="max-w-6xl w-full mx-auto px-6 py-8">
      <header className="flex items-end justify-between mb-6">
        <div>
          <SectionLabel className="mb-1">Gestion</SectionLabel>
          <h1 className="text-2xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">Financeurs</h1>
          <p className="text-[13px] text-zinc-500 dark:text-zinc-400 mt-1">{funders.length} financeurs configurés</p>
        </div>
        <Link
          href="#"
          className="bg-violet-600 hover:bg-violet-700 text-white text-[13px] font-medium px-4 py-2 rounded-md transition shadow-sm inline-flex items-center gap-2"
        >
          <Plus className="w-3.5 h-3.5" />
          Nouveau financeur
        </Link>
      </header>
      <ul className="border-y border-zinc-200/60 dark:border-zinc-800 divide-y divide-zinc-200/60 dark:divide-zinc-800">
        {funders.map((f) => (
          <li key={f.id} className="grid grid-cols-[1fr_140px_120px] gap-3 py-3 px-1 items-center text-[13px]">
            <span className="text-zinc-900 dark:text-zinc-100">{f.name}</span>
            <span className="text-[11px] bg-zinc-100 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 px-2 py-0.5 rounded font-mono w-fit">
              {labels[f.kind]}
            </span>
            <span className="font-mono text-[11px] text-zinc-500">—</span>
          </li>
        ))}
      </ul>
    </div>
  );
}
