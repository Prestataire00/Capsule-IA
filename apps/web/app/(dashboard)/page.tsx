// ARCHETYPE: command
// Justification: home dashboard chaleureuse — accueil personnalisé, hiérarchie marquée, action principale brand.
...
import {
  ...
  FileSignature, Calendar, Sparkles,
} from 'lucide-react';
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)]">
      <div className="max-w-6xl w-full mx-auto px-8 py-10">
        ...
        {/* Hero — gradient subtil orange autorisé sur l'accueil */}
        <header className="relative overflow-hidden bg-gradient-to-br from-orange-50 via-white to-zinc-50 dark:from-orange-950/30 dark:via-zinc-950 dark:to-zinc-900 border border-orange-100/50 dark:border-orange-950/30 rounded-xl px-7 py-7 mb-10 shadow-sm">
          <div className="absolute -top-12 -right-12 w-48 h-48 bg-orange-200/40 dark:bg-orange-900/20 rounded-full blur-3xl pointer-events-none" />
          <div className="relative">
            <p className="text-[11px] tracking-wider uppercase text-orange-700 dark:text-orange-400 mb-2 font-medium">
              {format(today, 'EEEE d MMMM yyyy', { locale: fr })}
            </p>
            <h1 className="text-3xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">
              {greet()}, {firstName}.
            </h1>
            <p className="text-[15px] text-zinc-600 dark:text-zinc-400 mt-3 max-w-2xl">
              {kpis.active > 0
            ...
            <div className="mt-5 flex items-center gap-3">
              ...
              <Link
                href="/dossiers"
                className="text-[13px] text-zinc-700 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-zinc-100 px-3 py-2 transition inline-flex items-center gap-1.5"
              >
        ...
        <section className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
          <StatCard label="Dossiers actifs" value={kpis.active} icon={FolderOpen} hint="↑ 2 ce mois" hintTone="success" />
          <StatCard
        ...
        <section className="mb-10">
          <div className="flex items-end justify-between mb-4">
            <div>
              <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">Prochaines sessions</h2>
          ...
          {upcomingSessions.length === 0 ? (
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 rounded-xl py-12 text-center shadow-sm">
              ...
              {upcomingSessions.map((s) => {
                ...
                return (
                  <Link
                    ...
                    className="group bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 rounded-xl px-5 py-4 hover:border-orange-300 dark:hover:border-orange-900/60 hover:shadow-md transition shadow-sm"
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div>
                        <p className="text-[11px] tracking-wider uppercase text-orange-700 dark:text-orange-400 font-medium">
                          {format(parseISO(s.startsAt), 'EEEE d MMMM', { locale: fr })}
                        </p>
                        <p className="font-mono text-[13px] text-zinc-900 dark:text-zinc-100 mt-1">
                          {format(parseISO(s.startsAt), 'HH:mm')} – {format(parseISO(s.endsAt), 'HH:mm')}
                        </p>
                      </div>
                      <ArrowUpRight className="w-4 h-4 text-zinc-300 dark:text-zinc-600 group-hover:text-orange-500 transition" />
                    </div>
                    <p className="text-[15px] font-medium text-zinc-900 dark:text-zinc-100 truncate mt-2">
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-10">
          <section>
            <div className="flex items-end justify-between mb-4">
              <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">À traiter</h2>
              <span className="text-[11px] text-zinc-500 dark:text-zinc-400">{tasks.length}</span>
            </div>
            {tasks.length === 0 ? (
              <div className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 rounded-xl py-10 text-center shadow-sm">
              ...
              <ul className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 rounded-xl divide-y divide-zinc-200/60 dark:divide-zinc-800 shadow-sm overflow-hidden">
                {tasks.map((t, i) => {
                  const Icon = t.icon;
                  const styles = toneStyles[t.tone];
                  return (
                    <li key={i}>
                      <Link href={t.href} className="flex items-start gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-950 transition group">
                        ...
                        <ArrowUpRight className="w-3.5 h-3.5 text-zinc-300 dark:text-zinc-700 group-hover:text-orange-500 transition flex-shrink-0 mt-1" />
                      </Link>
                    </li>
                  );
                })}
          ...
          <section>
            <div className="flex items-end justify-between mb-4">
              <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">Dossiers récents</h2>
              <Link
                href="/dossiers"
                className="text-[13px] text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 inline-flex items-center gap-1 transition"
              >
            ...
            <ul className="bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 rounded-xl divide-y divide-zinc-200/60 dark:divide-zinc-800 shadow-sm overflow-hidden">
              {dossiers.slice(0, 5).map((d) => (
                <li key={d.id}>
                  <Link href={`/dossiers/${d.id}`} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-950 transition group">
