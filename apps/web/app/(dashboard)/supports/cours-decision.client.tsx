...
import { deciderCours } from './cours-actions';

/** Valider ou refuser un contenu pédagogique. Le refus exige son motif. */
export function CoursDecision({ exerciseId }: { exerciseId: string }) {
  ...
  const decider = (decision: 'valide' | 'refuse', reason?: string) => {
    ...
    startTransition(async () => {
      const res = await deciderCours({ exerciseId, decision, reason });
    ...
    return (
      <div className="w-full space-y-2">
        <textarea
          ...
          maxLength={1000}
          autoFocus
          placeholder="Motif du refus (ex. question 3 : la bonne réponse est fausse)"
