...
export function FinalizeButton({ sheetId, initialFinalized, initialDocumentId, allSigned }: Props) {
  ...
  const handleFinalize = () => {
    ...
    startTransition(async () => {
      const r = await finalizeAttendanceSheet({ sheetId, actorUserId: null });
      if (!r.ok) {
        setError(r.error);
        return;
      }
    ...
    <section className="mt-6 bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 rounded-lg p-4">
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          title={!allSigned ? 'Toutes les signatures doivent être complètes' : undefined}
