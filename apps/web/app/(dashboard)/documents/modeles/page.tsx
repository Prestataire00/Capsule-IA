...
export default async function ModelesPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-6 space-y-8">
      ...
      {rows.length === 0 ? (
        <div className="text-center py-12 space-y-2 border border-dashed border-zinc-200/70 dark:border-zinc-800 rounded-xl">
          ...
          <p className="text-[12px] text-zinc-400">Importez les modèles par défaut ou créez-en un.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {groups.map((g) => (
            <div key={g.id ?? 'none'} className="space-y-1.5">
              <p className="text-[11px] uppercase tracking-wider text-zinc-400">{g.name}</p>
              {g.items.length === 0 ? (
                <p className="text-[12px] text-zinc-400 italic px-1">Aucun modèle.</p>
              ) : (
