  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      ...
      <Link
        ...
        className="group flex items-center gap-4 bg-white dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-800 rounded-xl shadow-sm p-5 mb-6 hover:border-orange-300 dark:hover:border-orange-800 hover:shadow-md transition"
      >
        ...
        <ArrowRight className="w-4 h-4 text-zinc-400 group-hover:text-orange-500 transition flex-shrink-0" />
      </Link>
        ...
        <div className="p-6">
          <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
            <div className="flex items-start gap-3 flex-1 min-w-0">
              <GraduationCap className="w-5 h-5 mt-1 text-zinc-400 flex-shrink-0" />
              <div className="min-w-0">
                <p className="font-mono text-[11px] text-zinc-400 dark:text-zinc-500">{ctx.dossier.reference}</p>
                <p className="text-[17px] font-extrabold text-zinc-900 dark:text-zinc-100 truncate">{ctx.formation?.title ?? '—'}</p>
          ...
          <div className="mt-4">
            <div className="flex items-baseline justify-between mb-1.5">
              ...
              <p className="text-[13px] text-zinc-900 dark:text-zinc-100 font-extrabold tabular-nums">{progress} %</p>
            </div>
      ...
      {nextSession && (
        <section className="bg-white dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-800 rounded-xl shadow-sm p-5 mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
          <Calendar
            className={`w-5 h-5 flex-shrink-0 ${
              nextSession.status === 'in_progress' ? 'text-orange-500 animate-pulse' : 'text-zinc-400'
            }`}
          />
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[15px] font-bold text-zinc-900 dark:text-zinc-100 capitalize truncate tabular-nums mt-1">
      ...
      {/* Carte Assiduité */}
      <section className="bg-white dark:bg-zinc-900 border border-zinc-200/70 dark:border-zinc-800 rounded-xl shadow-sm p-5 mb-8 flex items-center gap-4">
        <BookOpen className="w-5 h-5 text-zinc-400 flex-shrink-0" />
        <div className="flex-1 min-w-0">
...
// Charte v4 : plus de pastille d'icône ; l'accent ne teinte que la ligne d'état de la tuile.
